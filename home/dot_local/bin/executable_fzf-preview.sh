...
if [[ ! $type =~ image/ ]]; then
  # Check for directories first, before checking for binary files
  if [[ -d $file ]]; then
    if command -v eza >/dev/null; then
      eza --color=always --icons -T "$file" | head -200
    elif command -v ls >/dev/null; then
      ls -la --color=always "$file"
    else
      echo "Directory: $file"
      find "$file" -maxdepth 1 -type f | head -20
    fi
    exit
  fi

  if [[ $type =~ application/.*binary ]] || [[ $type =~ application/octet-stream ]]; then
    file "$1"
    exit
  fi
