...
Singleton {
    ...
    // Debug flag to control console verbosity
    ...
    // Night mode properties - following Dank implementation
    ...
    property int fromHour: {
        if (!SessionData.nightModeFromTime)
            return 19;
        return Number(SessionData.nightModeFromTime.split(":")[0]);
    }
    property int fromMinute: {
        if (!SessionData.nightModeFromTime)
            return 0;
        return Number(SessionData.nightModeFromTime.split(":")[1]);
    }
    property int toHour: {
        if (!SessionData.nightModeToTime)
            return 6;
        return Number(SessionData.nightModeToTime.split(":")[0]);
    }
    property int toMinute: {
        if (!SessionData.nightModeToTime)
            return 30;
        return Number(SessionData.nightModeToTime.split(":")[1]);
    }
    ...
    property int clockMinute: {
        const now = new Date();
        return now.getMinutes();
    }

    // Watch for time changes and automatic mode changes
    Timer {
        id: timeWatcher
        ...
        onTriggered: {
            ...
            if (newHour !== root.clockHour || newMinute !== root.clockMinute) {
                root.clockHour = newHour;
                root.clockMinute = newMinute;
                reEvaluate();
            }
        }
    }
    ...
    Connections {
        ...
        function onNightModeFromTimeChanged() {
            reEvaluate();
        }
        function onNightModeToTimeChanged() {
            reEvaluate();
        }
    }
    ...
    function ensureState() {
        if (!SessionData.nightModeAutomatic || root.manualActive !== undefined)
            return;

        if (root.shouldBeOn) {
            enableNightModeInternal();
        } else {
            disableNightModeInternal();
        }
    }
    ...
    function enableNightModeInternal() {
        nightModeActive = true;
        SessionData.setNightModeEnabled(true);

        const temperature = SessionData.nightModeTemperature || 4500;
        Quickshell.execDetached(["bash", "-c", `pidof hyprsunset || hyprsunset -t ${temperature}`]);
    }

    function disableNightModeInternal() {
    ...
    function enableNightMode() {
        ...
        // Test if hyprsunset exists before enabling
        hyprsunsetTestProcess.running = true;
    }
    ...
    function updateNightModeTemperature(temperature) {
        ...
        if (nightModeActive) {
            disableNightModeInternal();
            // Use timer to ensure process stops before restarting
            nightModeRestartTimer.start();
        }
    }

    function disableNightMode() {
        disableNightModeInternal();
        // Clear manual override when manually disabling
        manualActive = undefined;
    }

    function toggleNightMode() {
        if (manualActive === undefined)
            manualActive = nightModeActive;
    ...
    Component.onCompleted: {
        ...
        // Initialize automatic mode evaluation
        reEvaluate();

        // Check if night mode was enabled on startup
        if (SessionData.nightModeEnabled && !SessionData.nightModeAutomatic) {
            enableNightModeInternal();
        }
    }

    // Timer for restarting night mode after temperature change
    Timer {
        id: nightModeRestartTimer
        interval: 500
        running: false
        repeat: false
        onTriggered: {
    ...
    Process {
        ...
        onExited: function (exitCode) {
            if (exitCode === 0) {
                // hyprsunset exists, enable night mode
                enableNightModeInternal();
            } else {
                // hyprsunset not found
                console.warn("BrightnessService: hyprsunset not found");
                ToastService.showWarning("Night mode failed: hyprsunset not found");
            }
        }
    }
    ...
    IpcHandler {
        ...
        function temperature(value: string): string {
            ...
            root.updateNightModeTemperature(rounded);
        ...
        function automatic(value: string): string {
            if (!value) {
                return "Automatic mode: " + (SessionData.nightModeAutomatic ? "enabled" : "disabled");
            }

            const enabled = value.toLowerCase() === "true" || value === "1" || value.toLowerCase() === "on";
            SessionData.setNightModeAutomatic(enabled);
            root.manualActive = undefined;
            root.firstEvaluation = true;
            root.reEvaluate();

            return "Automatic night mode: " + (enabled ? "enabled" : "disabled");
        }

        function times(from: string, to: string): string {
            if (!from && !to) {
                return `Current schedule: ${SessionData.nightModeFromTime} to ${SessionData.nightModeToTime}`;
            }

            if (from) {
                SessionData.setNightModeFromTime(from);
            }
            if (to) {
                SessionData.setNightModeToTime(to);
            }

            return `Night mode schedule set: ${SessionData.nightModeFromTime} to ${SessionData.nightModeToTime}`;
        }
